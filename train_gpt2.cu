...
// kept as a compatibility stub in llm.kittens; cuBLAS/cuBLASLt are not used.
...
#error "SM120 train_gpt2cu requires TK attention; do not build training with CUDA fallback attention"
#endif

#if defined(KITTENS_SM120) && LLMK_USE_TK_MHA_BWD && !defined(LLMK_SM120_ATOMIC_DQ)
#define LLMK_GPT2_USE_SM120_PACKED_QKV_ATTENTION 1
#else
#define LLMK_GPT2_USE_SM120_PACKED_QKV_ATTENTION 0
#endif
...
static const char* GPT2_PARAMETER_TENSOR_NAMES[NUM_PARAMETER_TENSORS] = {
    "GPT-2 wte",
    "GPT-2 wpe",
    "GPT-2 ln1w per layer",
    "GPT-2 ln1b per layer",
    "GPT-2 qkvw per layer",
    "GPT-2 qkvb per layer",
    "GPT-2 attprojw per layer",
    "GPT-2 attprojb per layer",
    "GPT-2 ln2w per layer",
    "GPT-2 ln2b per layer",
    "GPT-2 fcw per layer",
    "GPT-2 fcb per layer",
    "GPT-2 fcprojw per layer",
    "GPT-2 fcprojb per layer",
    "GPT-2 lnfw",
    "GPT-2 lnfb",
};
...
void* malloc_and_point_parameters(ParameterTensors* params, size_t* param_elements, size_t *param_sizeof) {
    ...
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
        assert(((uintptr_t)params_memory_iterator % 128) == 0 &&
...
typedef struct {
    ...
    floatX* matmul_scratch; // max dWeight tensor, for accumulated TK A^T*B
} ActivationTensors;
...
void fill_in_activation_sizes(const ActivationTensors* data, TensorSpec (&tensors)[NUM_ACTIVATION_TENSORS], size_t B, size_t T, GPT2Config config, int recompute) {
    ...
    size_t T_attn_pad = CEIL_DIV(T, llmk::attention::fwd_sequence_granularity())
                      * llmk::attention::fwd_sequence_granularity();
    size_t padded_attention_scratch = 4 * B * T_attn_pad * C
                                    + 2 * B * NH * T_attn_pad; // float LSE tail, in floatX elements
#if defined(KITTENS_SM120)
#if defined(LLMK_SM120_ATOMIC_DQ)
    size_t attention_backward_scratch = 4 * B * T * C
                                      + 2 * B * NH * T; // bf16 O/dO + float D/dQ, in floatX elements
#else
    size_t attention_backward_scratch = 2 * B * T * C
                                      + 2 * B * NH * T; // bf16 O/dO + float D, in floatX elements
#endif
#else
    size_t attention_backward_scratch = 8 * B * T * C
                                      + 2 * B * NH * T; // bf16 O/dO + float D/dQ/dK/dV, in floatX elements
#endif
    tensors[18] = TENSOR_SPEC(data->output,
                              max(B * T * max(3*C, max(NH*T, Vp)),
                                  max(padded_attention_scratch,
                                      attention_backward_scratch)));
...
#define LLMK_SM120_LARGE_DWEIGHT_SPLIT_K 8
    ...
    tensors[21] = TENSOR_SPEC(data->matmul_scratch,
                              max((size_t)LLMK_SM120_LARGE_DWEIGHT_SPLIT_K * Vp * C,
                                  (size_t)LLMK_SM120_DWEIGHT_SPLIT_K * 4 * C * C));
#else
    tensors[21] = TENSOR_SPEC(data->matmul_scratch, max(Vp * C, 4 * C * C));
#endif
}
    ...
    for (size_t i = 0; i < NUM_ACTIVATION_TENSORS; i++) {
        ...
        if(tensors[i].size == 0) {
            *(tensors[i].ptr) = NULL;
            ...
            uintptr_t aligned = ((uintptr_t)acts_memory_iterator + llmk::TK_ALIGN - 1)
                              / llmk::TK_ALIGN * llmk::TK_ALIGN;
            acts_memory_iterator = (char*)aligned;
            assert(((uintptr_t)acts_memory_iterator % llmk::TK_ALIGN) == 0 &&
...
void gpt2_fill_parameter_metadata(GPT2 *model) {
...
void gpt2_allocate_weights(GPT2 *model) {
    gpt2_fill_parameter_metadata(model);
    // create memory for model parameters on the device
    assert(model->params_memory == nullptr);
    model->params_memory = malloc_and_point_parameters(&model->params, model->param_elements, model->param_sizeof);
}

void gpt2_allocate_zero3_parameter_shards(GPT2 *model, MultiGpuConfig* config) {
    if (!zero_shards_parameters(config)) {
        return;
    }
    if (model->params_memory == NULL) {
        fprintf(stderr, "Need full GPT parameter buffer before initializing ZeRO-3 shards\n");
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 0; i < NUM_PARAMETER_TENSORS; ++i) {
        int num_layers = (2 <= i && i <= 13) ? model->config.num_layers : 1;
    ...
    printf0("allocated %zu MiB for ZeRO-3 GPT parameter shards\n",
...
void gpt2_config_from_checkpoint_header(GPT2Config* config, const char* checkpoint_path, bool weight_init=true) {
...
void gpt2_validate_checkpoint_payload_size(const GPT2* model, const char* checkpoint_path) {
    FILE *model_file = fopenCheck(checkpoint_path, "rb");
    fseekCheck(model_file, 0, SEEK_END);
    int64_t file_size_bytes = ftell(model_file);
    fcloseCheck(model_file);
    const int header_bytes = 256 * (int)sizeof(int);
    int64_t expected_file_size = (int64_t)header_bytes + (int64_t)model->num_parameters_bytes;
    if (file_size_bytes != expected_file_size) {
        fprintf(stderr,
                "Bad GPT checkpoint size for %s: got %lld bytes, expected %lld bytes "
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path, bool weight_init=true) {
    gpt2_config_from_checkpoint_header(&model->config, checkpoint_path, weight_init);

    // allocate memory for the model parameters
    gpt2_allocate_weights(model);
    gpt2_validate_checkpoint_payload_size(model, checkpoint_path);

    // read in the parameters if weight_init is true
    if (weight_init) {
        assert(model->params_memory != NULL);
        FILE *model_file = fopenCheck(checkpoint_path, "rb");
        fseekCheck(model_file, 256 * sizeof(int), SEEK_SET);
...
void gpt3_set_hyperparameters(GPT2Config* config, const char* channels_str) {
    ...
    else if (channels == 5120)  { depth = 40; head_size = 128; } // gpt3-13B
...
void gpt_config_from_descriptor(GPT2Config* config, const char* descriptor) {
...
void gpt_build_from_descriptor(GPT2 *model, const char* descriptor) {
    gpt_config_from_descriptor(&model->config, descriptor);
...
void gpt2_forward(GPT2 *model, const int* inputs, size_t B, size_t T) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        if (T != model->seq_len) { // unused parts of attention buffer must be zeroed (T-dependent)
            cudaCheck(cudaMemsetAsync(l_att, 0, 2 * B * NH * T * sizeof(floatX), main_stream));
        }
        ...
        attention_forward_packed_qkv(l_atty, l_att, l_qkvr, B, T, C, NH, main_stream);
#else
    ...
    matmul_forward(acts.output, acts.lnf, params.wte, NULL, B, T, C, Vp, main_stream);
    // Keep the forward pass asynchronous. Training immediately queues backward
    // on the same stream, and validation/generation synchronize through their
    // following device-to-host copies.
...
float gpt2_validate(GPT2 *model, const int* inputs, const int* targets, size_t B, size_t T) {
    ...
    cudaCheck(cudaMemsetAsync(acts.losses, 0, B*T*sizeof(float), main_stream));
...
void gpt2_backward_and_reduce(GPT2 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step) {
    ...
    if(last_step) {
        ...
        #if MULTI_GPU
        ncclCheck(ncclAllReduce(model->accumulated_mean_loss, model->accumulated_mean_loss, 1, ncclFloat, ncclAvg, multi_gpu_config.nccl_comm, main_stream));
        #endif
...
void gpt2_validate_zero_tensor_sharding(const GPT2* model, const MultiGpuConfig* config) {
    for (int i = 0; i < NUM_PARAMETER_TENSORS; ++i) {
        ShardInfo tensor = gpt2_get_tensor_at_layer(model, 0, i);
        validate_zero_tensor_shardable(GPT2_PARAMETER_TENSOR_NAMES[i], tensor.size, config);
    }
}
...
float gpt2_calculate_grad_norm(GPT2 *model, MultiGpuConfig* multi_gpu_config) {
    ...
    if (zero_uses_reduce_scatter_gradients(multi_gpu_config)) {
        ...
        ncclCheck(ncclAllReduce(grad_norm_squared, grad_norm_squared, 1, ncclFloat, ncclSum, multi_gpu_config->nccl_comm, main_stream));
#endif
    } else {
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, float grad_scale, int t,
    ...
    if (zero_shards_parameters(multi_gpu_config) && model->param_shards_memory == NULL) {
        fprintf(stderr, "Need to allocate ZeRO-3 GPT parameter shards before update\n");
        exit(EXIT_FAILURE);
    }
    ...
    if(init_state) {
        ...
        cudaCheck(cudaMemsetAsync(model->m_memory, 0, multi_gpu_config->shard_num_parameters * sizeof(float), main_stream));
        cudaCheck(cudaMemsetAsync(model->v_memory, 0, multi_gpu_config->shard_num_parameters * sizeof(float), main_stream));
    }
    ...
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
        ...
        floatX* param_ptr = zero_shards_parameters(multi_gpu_config)
        ...
        ptrdiff_t param_stride = zero_shards_parameters(multi_gpu_config) ? shard.size : tensor.size;
        ...
        if (zero_shards_parameters(multi_gpu_config)) {
            zero_all_gather_parameter_shards_to_full(
                ...
                multi_gpu_config,
                main_stream);
        } else if (zero_shards_optimizer_state(multi_gpu_config)) {
#if MULTI_GPU
            multi_gpu_sync_nccl_stream_from_compute(multi_gpu_config, main_stream);
...
MultiGpuConfig gpt2_host_only_multi_gpu_config(int num_processes, int process_rank, int gpus_per_node) {
    if (num_processes < 1) {
        fprintf(stderr, "Invalid process count %d for GPT dry run.\n", num_processes);
        exit(EXIT_FAILURE);
    }
    if (process_rank < 0 || process_rank >= num_processes) {
        fprintf(stderr, "Invalid process rank %d for %d processes in GPT dry run.\n",
                process_rank, num_processes);
        exit(EXIT_FAILURE);
    }
    if (gpus_per_node < 1) {
        fprintf(stderr, "Invalid GPUs-per-node count %d for GPT dry run.\n", gpus_per_node);
        exit(EXIT_FAILURE);
    }
...
void gpt2_load_metadata_for_dry_run(GPT2* model, const char* model_source, bool weight_init=true) {
    const bool is_checkpoint = ends_with_bin(model_source);
    if (is_checkpoint) {
        gpt2_config_from_checkpoint_header(&model->config, model_source, weight_init);
    } else {
        gpt_config_from_descriptor(&model->config, model_source);
    }
    gpt2_fill_parameter_metadata(model);
    if (is_checkpoint) {
        gpt2_validate_checkpoint_payload_size(model, model_source);
    }
}

void gpt2_dry_run_validate(const char* model_source, int resuming, int use_master_weights,
                           int gelu_fusion, int recompute, int T, int zero_stage,
                           MultiGpuConfig* dry_config) {
    GPT2 model;
    gpt2_init_common(&model);
    const bool weight_init = !(resuming == 1 && use_master_weights);
    gpt2_load_metadata_for_dry_run(&model, model_source, weight_init);
    model.use_master_weights = use_master_weights;
    model.gelu_fusion = gelu_fusion;
    model.recompute = recompute;

    if (T > model.config.max_seq_len) {
        fprintf(stderr, "GPT dry run sequence length T=%d exceeds model max_seq_len=%d.\n",
                T, model.config.max_seq_len);
        exit(EXIT_FAILURE);
    }

    set_zero_configs(dry_config, zero_stage, model.num_parameters);
    gpt2_validate_zero_tensor_sharding(&model, dry_config);
    printf0("GPT dry run: source=%s\n", model_source);
    printf0("GPT dry run: max_seq_len=%d vocab=%d padded_vocab=%d layers=%d heads=%d channels=%d\n",
    ...
    printf0("GPT dry run: parameters=%zu bytes=%zu\n",
            model.num_parameters, model.num_parameters_bytes);
    printf0("GPT dry run: ZeRO-%d layout validated for %d process(es).\n",
            dry_config->zero_stage, dry_config->num_processes);
}
...
int main(int argc, char *argv[]) {
    ...
    // Host-only dry run: parse model metadata and validate ZeRO shardability without CUDA/NCCL.
    if (max_steps == 0) {
        if (output_log_dir != NULL) {
            assert(strlen(output_log_dir) < 400); // careful bunch of hardcoded snprintf around this
        }
        MultiGpuConfig dry_config = gpt2_host_only_multi_gpu_config(num_processes, process_rank, gpus_per_node);
        multi_gpu_config = dry_config;

        int resuming = 0;
        int resume_max_step = -1;
        const char* model_source = load_filename;
        if (resume == 1) {
            assert(output_log_dir != NULL);
            resume_max_step = find_max_step(output_log_dir);
            if (resume_max_step != -1) {
                resuming = 1;
                snprintf(filename_buffer, sizeof(filename_buffer), "%s/model_%08d.bin", output_log_dir, resume_max_step);
                model_source = filename_buffer;
            }
        }
        ...
        gpt2_dry_run_validate(model_source, resuming, use_master_weights,
                              gelu_fusion, recompute, T, zero_stage, &dry_config);
        return EXIT_SUCCESS;
    }
    ...
    // SM120 has a fast enough MLP-up GELU epilogue to use by default.
    // Manual -ge still overrides this.
    if (gelu_fusion == -1) {
#if defined(KITTENS_SM120)
        gelu_fusion = 1;
#else
    ...
    gpt2_validate_zero_tensor_sharding(&model, &multi_gpu_config);
