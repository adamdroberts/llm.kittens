...
// Ported from llm.c/test_gpt2.cu. The gradient tolerance table below keeps the
// inherited llm.c BF16 thresholds explicit, so the first H100 TK parity run can
// update individual tensor classes instead of relaxing one anonymous array.
...
typedef struct {
    const char* name;
    float upstream_bf16_threshold;
    float tk_bf16_threshold;
    const char* note;
} GradientTolerance;

static_assert(NUM_PARAMETER_TENSORS == 16, "Update GPT-2 gradient tolerance table");

static const GradientTolerance kGradientTolerances[NUM_PARAMETER_TENSORS] = {
    {"wte",       5e-1f,   5e-1f,   "embedding gradient; sparse token hits dominate"},
    {"wpe",       4e-3f,   4e-3f,   "embedding gradient"},
    {"ln1w",      1e-1f,   1e-1f,   "layernorm scale"},
    {"ln1b",      4e-2f,   4e-2f,   "layernorm bias"},
    {"qkvw",      5e-2f,   5e-2f,   "attention input projection; likely TK MHA-bwd-sensitive"},
    {"qkvb",      3.5e-2f, 3.5e-2f, "attention input projection bias; likely TK MHA-bwd-sensitive"},
    {"attprojw",  2e-2f,   2e-2f,   "attention output projection; likely TK MHA-bwd-sensitive"},
    {"attprojb",  3e-2f,   3e-2f,   "attention output projection bias"},
    {"ln2w",      5e-2f,   5e-2f,   "layernorm scale"},
    {"ln2b",      3e-2f,   3e-2f,   "layernorm bias"},
    {"fcw",       3e-2f,   3e-2f,   "MLP up projection"},
    {"fcb",       3e-2f,   3e-2f,   "MLP up bias"},
    {"fcprojw",   2e-2f,   2e-2f,   "MLP down projection"},
    {"fcprojb",   1e-2f,   1e-2f,   "MLP down bias"},
    {"lnfw",      1e-1f,   1e-1f,   "final layernorm scale"},
    {"lnfb",      2e-2f,   2e-2f,   "final layernorm bias"},
};

static float gradient_threshold_for_tensor(int tensor_id) {
    #if defined(ENABLE_FP32)
    (void)tensor_id;
    return 1e-6f;  // we can be much more precise in FP32
    #else
    return kGradientTolerances[tensor_id].tk_bf16_threshold;
    #endif
}
...
int main(int argc, char *argv[]) {
    ...
    for (int step = 0; step < 10; step++) {
        ...
        if (step == 0) {
            ...
            // Compare gradients tensor-by-tensor. The TK thresholds currently
            // match the inherited llm.c BF16 values; once a real H100 run
            // produces measured TK MHA-bwd drift, update kGradientTolerances
            // one row at a time and keep the upstream baseline visible.
            size_t* count = model.param_elements;
            for(int i = 0; i < NUM_PARAMETER_TENSORS; ++i) {
                const GradientTolerance& tol = kGradientTolerances[i];
                printf("gradient tolerance: %s upstream_bf16=%g tk_bf16=%g (%s)\n",
                       tol.name, tol.upstream_bf16_threshold, tol.tk_bf16_threshold, tol.note);
                allok = allok & check_tensor(
                    tensors1[i], tensors2[i], count[i], tol.name, gradient_threshold_for_tensor(i));
            }
        }
