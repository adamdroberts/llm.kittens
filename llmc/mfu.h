...
// RTX 5090 dense spec-sheet TFLOPS (5th-gen tensor, GB202): TF32 209.5,
// BF16/FP16 fp32-acc 419, FP16 fp16-acc 838, FP8 fp32-acc 838, FP8 fp16-acc 1676.
// CLOCK / CORES below are the 5090's own boost-clock / TC count, so the
// gpu_db entry uses identity scaling. Other Blackwell consumer SKUs scale
// from these via (new_cores / 680) * (new_mhz / 2407).
static const PerfData BLACKWELL_CONSUMER = {209.5f, 419.f, 419.f, 838.f, 838.f, 1676.f, 2407.f, 680.f};
...
const char* get_throttle_reason(unsigned long long bits) {
    if(bits & (nvmlClocksEventReasonSwPowerCap | nvmlClocksThrottleReasonHwPowerBrakeSlowdown)) {
        return "power cap";
    } else if (bits & (nvmlClocksEventReasonSwThermalSlowdown | nvmlClocksThrottleReasonHwThermalSlowdown)) {
        return "thermal cap";
    } else if (bits & (nvmlClocksEventReasonAll)) {
        return "other cap";
    } else {
        return "no cap";
    }
...
GPUUtilInfo get_gpu_utilization_info() {
    ...
    nvmlCheck(nvmlDeviceGetTemperatureV(device, &temperature));
    info.temperature = (unsigned int)temperature.temperature;
