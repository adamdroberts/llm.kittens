...
void multi_gpu_barrier(const MultiGpuConfig* config) {
    ...
    if (config->num_processes > 1) {
        ncclCheck(ncclAllReduce(config->unified_buffer, config->unified_buffer, 1, ncclFloat, ncclSum, config->nccl_comm, config->nccl_stream));
    }
    cudaCheck(cudaDeviceSynchronize());
#endif
}

// Make NCCL stream work depend on kernels already queued on compute_stream.
// Use before collectives that read buffers just produced by model/optimizer kernels.
void multi_gpu_sync_nccl_stream_from_compute(MultiGpuConfig* config, cudaStream_t compute_stream) {
#ifdef MULTI_GPU
    if (config->num_processes <= 1) {
        return;
    }
...
static inline bool zero_shards_optimizer_state(const MultiGpuConfig* config) {
    return config->zero_stage >= 1;
}

static inline bool zero_uses_reduce_scatter_gradients(const MultiGpuConfig* config) {
    return config->zero_stage >= 1;
}

static inline bool zero_shards_parameters(const MultiGpuConfig* config) {
    return config->zero_stage >= 3;
}

template<typename T>
__global__ void zero_copy_strided_kernel(T* dst, const T* src, size_t n,
                                         ptrdiff_t dst_stride,
                                         ptrdiff_t src_stride) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        dst[idx + dst_stride * blockIdx.y] = src[idx + src_stride * blockIdx.y];
    }
}

template<typename T>
void zero_copy_parameter_shards_from_full(T* dst_shards, const T* src_full,
                                          size_t shard_size,
                                          ptrdiff_t shard_stride,
                                          ptrdiff_t full_stride,
                                          int num_slices,
                                          cudaStream_t stream) {
    int block_size = 512;
    int grid_size = CEIL_DIV(shard_size, block_size);
    zero_copy_strided_kernel<<<dim3(grid_size, num_slices), block_size, 0, stream>>>(
        dst_shards, src_full, shard_size, shard_stride, full_stride);
    cudaCheck(cudaGetLastError());
}

template<typename T>
void zero_all_gather_parameter_shards_to_full(T* dst_full, T* src_shards,
                                             size_t shard_size,
                                             ptrdiff_t full_stride,
                                             ptrdiff_t shard_stride,
                                             int num_slices,
    ...
    if (config->num_processes == 1) {
        zero_copy_parameter_shards_from_full(
            dst_full, src_shards, shard_size, full_stride, shard_stride,
            num_slices, compute_stream);
        return;
    }

#ifdef MULTI_GPU
    multi_gpu_sync_nccl_stream_from_compute(config, compute_stream);
    ncclCheck(ncclGroupStart());
    for (int l = 0; l < num_slices; ++l) {
        ncclCheck(ncclAllGather(src_shards + l * shard_stride,
                                dst_full + l * full_stride,
                                shard_size,
                                ncclFloatX,
                                config->nccl_comm,
                                config->nccl_stream));
...
#else
    fprintf(stderr, "ZeRO-3 parameter all-gather requires MULTI_GPU when num_processes=%d.\n",
            config->num_processes);
    exit(EXIT_FAILURE);
#endif
}

static inline void validate_zero_tensor_shardable(const char* tensor_name, size_t elements,
                                                  const MultiGpuConfig* config) {
    if (config->num_processes < 1) {
        fprintf(stderr, "Invalid ZeRO process count %d for %s.\n", config->num_processes, tensor_name);
        exit(EXIT_FAILURE);
    }
    if (!zero_uses_reduce_scatter_gradients(config) || config->num_processes == 1) {
        return;
    }
    if (elements % config->num_processes != 0) {
        fprintf(stderr,
                "ZeRO stage %d cannot shard %s: %zu elements is not divisible "
                "by %d processes.\n",
                config->zero_stage, tensor_name, elements, config->num_processes);
        exit(EXIT_FAILURE);
    }
}
...
ShardInfo multi_gpu_get_shard_offset(size_t elements, const MultiGpuConfig* config, int shard_at_stage) {
    ...
    if (nproc < 1) {
        fprintf(stderr, "Invalid ZeRO process count %d while sharding %zu elements.\n", nproc, elements);
        exit(EXIT_FAILURE);
    }
...
// This can work either as an all-reduce (ZeRO-0), or a reduce-scatter (ZeRO-1/2).
// Stage 2 uses the same reduced shard values as stage 1, while the trainer still
// keeps full gradient accumulation buffers because the backward kernels write into
// the full parameter layout.
...
void multi_gpu_async_reduce_gradient(
    ...
    for (int i = 0; i < N; ++i) {
        ...
        } else if(zero_uses_reduce_scatter_gradients(config)) {
            if (pointers_sizes[i] % config->num_processes != 0) {
                fprintf(stderr,
                        "ZeRO stage %d reduce-scatter input %d has %zu elements, "
                        "which is not divisible by %d processes.\n",
                        config->zero_stage, i, pointers_sizes[i], config->num_processes);
                exit(EXIT_FAILURE);
            }
...
static inline void validate_zero_stage_request(int zero_stage) {
    if (zero_stage >= 0 && zero_stage <= 3) {
        return;
    }
    fprintf(stderr,
            "ZeRO stage %d was requested, but llm.kittens currently supports only "
            "ZeRO-0, ZeRO-1, ZeRO-2, and ZeRO-3. "
            "Refusing to silently disable sharding.\n",
            zero_stage);
    exit(EXIT_FAILURE);
}

static inline void validate_zero_runtime_request(int zero_stage) {
...
void set_zero_configs(MultiGpuConfig* config, int zero_stage, size_t total_parameters) {
    validate_zero_stage_request(zero_stage);
    if (config->num_processes < 1) {
        fprintf(stderr, "Invalid ZeRO process count %d.\n", config->num_processes);
        exit(EXIT_FAILURE);
    }
    config->zero_stage = 0;
    ...
    else if (zero_stage == 1 || zero_stage == 2 || zero_stage == 3) {
        if (total_parameters % config->num_processes != 0) {
            fprintf(stderr,
                    "ZeRO stage %d was requested, but %zu parameters cannot be evenly "
                    "partitioned across %d processes.\n",
                    zero_stage, total_parameters, config->num_processes);
            exit(EXIT_FAILURE);
        }
        else {
            ...
            if (zero_stage == 1) {
                printf0("| ZeRO Stage 1: optimizer states sharded; gradients reduce-scattered        |\n");
            } else {
                if (zero_stage == 2) {
                    printf0("| ZeRO Stage 2: optimizer states sharded; gradients reduce-scattered        |\n");
                    printf0("| Note: full gradient accumulation buffers are still allocated by kernels    |\n");
                } else {
                    printf0("| ZeRO Stage 3: parameter shards + runtime all-gather compute layout        |\n");
                    printf0("| Note: full compute/gradient buffers are still allocated by kernels        |\n");
                }
            }
        }
    }
...
float multi_gpu_cpu_float_sum(float value, MultiGpuConfig* config) {
    ...
    *unified_buffer = value;
    ncclCheck(ncclAllReduce(unified_buffer, unified_buffer, 1, ncclFloat, ncclSum, config->nccl_comm, config->nccl_stream));
    cudaCheck(cudaDeviceSynchronize());
