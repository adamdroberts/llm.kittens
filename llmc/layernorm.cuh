...
Ported from llm.c/llmc/layernorm.cuh as the correctness baseline for the
GPT-2 trainer API. Forward and fused-residual forward now dispatch to the
ThunderKittens fork for supported hidden widths; the CUDA path remains as the
fallback and backward baseline.
...
__global__ void __launch_bounds__(512, 2) // todo - any warnings on Turing with only 1024 threads?
    ...
    float* tk_reduce_shared = shared + 2 * rounded_C + 2 * (BLOCK_SIZE - WARP_SIZE) * f128::size;
    float* tk_reduce_warp = tk_reduce_shared + warpId * WARP_SIZE;
...
static inline void layernorm_forward_cuda(floatX* out, float* mean, float* rstd,
...
static inline void fused_residual_forward5_cuda(floatX* residual, floatX* normed, float* mean, float* rstd,
                                                const floatX* inp1, const floatX* inp2,
                                                const floatX* weight, const floatX* bias,
    ...
    if(status == cudaSuccess) {
        ...
        layernorm_forward_cuda(normed, mean, rstd, residual, weight, bias, N, 1, C, stream);
    }
    cudaCheck(cudaGetLastError());
}
...
void layernorm_forward(floatX* out, float* mean, float* rstd,
    ...
    if (llmk::layernorm::supports_width(C)) {
        llmk::layernorm::launch_forward(
            ...
            llmk::to_bf16(inp), llmk::to_bf16(weight), llmk::to_bf16(bias),
            B * T, C, stream);
        cudaCheck(cudaGetLastError());
        return;
    }
...
void fused_residual_forward5(floatX* residual, floatX* normed, float* mean, float* rstd,
    ...
    if (llmk::layernorm::supports_width(C)) {
        llmk::layernorm::launch_fused_residual_forward(
            ...
            llmk::to_bf16(inp1), llmk::to_bf16(inp2),
            llmk::to_bf16(weight), llmk::to_bf16(bias),
            N, C, stream);
        cudaCheck(cudaGetLastError());
        return;
    }
...
void layernorm_backward(floatX* dinp, floatX* dweight, floatX* dbias, float* scratch,
    ...
    size_t shared_mem_size = (2 * rounded_C + 2 * (block_size - 32) * f128::size
                           + (block_size / WARP_SIZE) * WARP_SIZE) * sizeof(float);

    cudaCheck(cudaMemsetAsync(scratch, 0, 1 * sizeof(float), stream)); // only need to reset the flag to 0
    cudaCheck(cudaFuncSetAttribute(layernorm_backward_kernel10,
