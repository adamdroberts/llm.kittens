// probe_adamw_ref.cu — runs llm.c's adamw_update for one step.
// CLI: probe_adamw_ref <io_dir>
//   reads:  param.npy (uint16 bf16), master.npy (float32), grad.npy (uint16 bf16),
//           m.npy (float32), v.npy (float32), shape.npy (int32 [N])
//   writes: ref/{param, master, m, v}.npy
//
// Hyperparameters are hard-coded to match probe_adamw_tk.cu so both probes
...
// llm.c's adamw.cuh defines a free `lerp(float,float,float)` that collides
// with C++20's `std::lerp` (visible via <cmath> pulled in by cuda_common.h).
// llm.kittens already renamed it to `llmc_lerp`; here we preprocess-rename
// llm.c's so this TU only sees one symbol.
#define lerp llmc_ref_lerp
