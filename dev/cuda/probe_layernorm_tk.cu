// probe_layernorm_tk.cu — runs llm.kittens's layernorm_forward (TK on H100,
// CUDA fallback elsewhere) on bf16 inputs and dumps outputs. Paired with
// probe_layernorm_ref.cu for the side-by-side parity test.
//
// CLI: probe_layernorm_tk <io_dir>
//   reads:  <io_dir>/{inp,weight,bias}.npy   (bf16 bits as uint16)
//           <io_dir>/shape.npy               (int32: [B, T, C])
//   writes: <io_dir>/tk/{out,mean,rstd}.npy
//
// Build: `make probe_layernorm_tk`.
#include <algorithm>
#include <cassert>
#include <cstdio>
#include <cstdlib>
