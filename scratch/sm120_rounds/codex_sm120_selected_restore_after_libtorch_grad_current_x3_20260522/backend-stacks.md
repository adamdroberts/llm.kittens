...
| GPU runtime | available | runtime timing and correctness execution | process-local GPU metadata probe returned no device details; this is not a target runtime availability signal<br>runtime availability is proven by explicit SM120 correctness and benchmark logs | use explicit correctness and benchmark logs as the runtime evidence source |
