...
- Rejected a temporary fused-forward-only swizzle hook with
  ...
  (all three smoke shapes), and the focused benchmark gave a local fused FC
  forward win in that run (`1519.42 us` TK versus `1554.88 us` cuBLASLt).
  It still left qkv forward/dInput/dWeight, attention-projection forward and
  dWeight, FC-projection forward+dInput, and LM-head rows behind the CUDA
  variants. TinyStories 3-step validation averaged `2623.14 ms` with steps
  `2618.96`, `2622.59`, and `2623.69 ms`, slower than the promoted source
  default and CUDA fallback diagnostics, so the temporary hook was removed.
- Rejected a temporary fused-forward-only swizzle hook with
  `LLMK_SM120_FORWARD_GELU_SUPER_M=2`, scoped to the `*_nt_bias_gelu` aliases
  while leaving non-GeLU forward, dInput, and dWeight aliases on their source
  defaults. The candidate passed `test_matmul` (`10/10`) and `test_attention`
  (all three smoke shapes), but the focused benchmark still left fused FC
  forward slightly behind cuBLASLt (`1524.09 us` TK versus `1511.10 us`) and
  left qkv forward/dWeight, attention-projection dWeight, FC-projection
  forward+dInput, and LM-head rows behind the CUDA variants. TinyStories
  3-step validation averaged `2624.24 ms` with steps `2619.59`, `2623.00`,
  and `2625.48 ms`, slower than the promoted source default and CUDA fallback
  diagnostics, so the temporary hook was removed.
- Rejected a temporary attention-projection-forward-only swizzle hook with
  `LLMK_SM120_ATTPROJ_FORWARD_SUPER_M=1`. The source hook routed only GPT-2
  ...
  benchmark gave an isolated attention-projection forward win in that run
  (`374.52 us` TK versus `425.92 us` cuBLASLt). It still left qkv
  forward/dInput/dWeight, attention-projection dWeight, FC/FC-projection
  dInput, and LM-head rows behind the CUDA variants. TinyStories 3-step
  validation averaged `2623.78 ms` with steps `2621.76`, `2621.62`, and
  `2625.93 ms`, slower than the promoted source default and CUDA fallback
  diagnostics, so the temporary hook was removed.
- Rejected a temporary attention-projection-forward-only swizzle hook with
  `LLMK_SM120_ATTPROJ_FORWARD_SUPER_M=2`. The source hook routed only GPT-2
  attention-projection forward (`N == 768`, `K == 768`) through a separate
  N96 forward+bias alias while leaving qkv, fused FC, FC-projection, LM-head,
  dInput, and dWeight dispatch unchanged. The candidate passed `test_matmul`
  (`10/10`) and `test_attention` (all three smoke shapes), but the focused
  benchmark still left attention-projection forward behind cuBLASLt
  (`374.63 us` TK versus `367.62 us`) and regressed qkv forward,
  attention-projection dWeight, FC-projection forward+dInput, and LM-head
  rows. TinyStories 3-step validation averaged `2631.54 ms` with steps
  `2632.35`, `2631.82`, and `2631.27 ms`, slower than the promoted source
  default and CUDA fallback diagnostics, so the temporary hook was removed.
- Rejected a temporary attention-projection-forward-only swizzle hook with
  `LLMK_SM120_ATTPROJ_FORWARD_SUPER_M=3`. The source hook routed only GPT-2
  attention-projection forward (`N == 768`, `K == 768`) through a separate
  N96 forward+bias alias while leaving qkv, fused FC, FC-projection, LM-head,
  dInput, and dWeight dispatch unchanged. The candidate passed `test_matmul`
  (`10/10`) and `test_attention` (all three smoke shapes), but the focused
  benchmark left the target row behind cuBLASLt (`374.76 us` TK versus
  `370.46 us`) and still left qkv forward/dWeight, attention-projection
  dWeight, FC/FC-projection, and LM-head rows behind the CUDA variants.
  TinyStories 3-step validation averaged `2634.78 ms` with steps `2626.25`,
  `2631.25`, and `2638.31 ms`, slower than the promoted source default and
...
- Rejected a temporary FC-projection-forward-only swizzle hook with
  `LLMK_SM120_FCPROJ_FORWARD_SUPER_M=1`. The source hook routed only GPT-2
  FC-projection forward (`N == 768`, `K == 3072`) through a separate N96
  forward+bias alias while leaving qkv, fused FC, LM-head, dInput, and dWeight
  dispatch unchanged. The candidate passed `test_matmul` (`10/10`) and
  `test_attention` (all three smoke shapes), but the focused benchmark
  regressed the target row to `1455.42 us` TK versus `1377.59 us` cuBLASLt
  and still left qkv, attention-projection dWeight, FC/FC-projection
  dInput, and LM-head rows behind. TinyStories 3-step validation averaged
  `2631.52 ms` with steps `2631.96`, `2632.10`, and `2630.94 ms`, slower
  than the promoted source default and CUDA fallback diagnostics, so the
  temporary hook was removed.
- Rejected a temporary qkv-forward-only swizzle route with
  `LLMK_SM120_QKV_FORWARD_SUPER_M=1`. The guarded hook routed only regular
  GPT-2 qkv forward GEMMs (`N == 3*K`) through a dedicated N96 forward+bias
  alias while leaving fused FC, LM-head, dInput, and dWeight dispatch
  unchanged. The candidate passed `test_matmul` (`10/10`) and
  `test_attention` (all three smoke shapes), but the focused benchmark
  regressed qkv forward to `1120.32 us` TK versus `1033.24 us` cuBLASLt and
  still left qkv dWeight, attention-projection dWeight, FC/FC-projection
  forward+dInput, and LM-head rows behind the CUDA variants. TinyStories
  3-step validation averaged `2625.17 ms` with steps `2625.31`, `2623.36`,
  and `2626.97 ms`, slower than the promoted source default and CUDA fallback
  diagnostics, so the temporary hook was removed.
- Rejected a temporary fused-forward-only swizzle hook with
  `LLMK_SM120_FORWARD_GELU_SUPER_M=6`, scoped to the `*_nt_bias_gelu` aliases
  while leaving non-GeLU forward, dInput, and dWeight aliases on their source
  defaults. The candidate passed `test_matmul` (`10/10`) and `test_attention`
  (all three smoke shapes), and the focused benchmark gave a local fused FC
  forward win in that run (`1525.64 us` TK versus `1563.88 us` cuBLASLt).
  However, qkv forward/dWeight, attention-projection dWeight, FC-projection,
  and LM-head rows still trailed cuBLASLt; TinyStories 3-step validation
  averaged `2624.92 ms` with steps `2620.06`, `2628.01`, and `2626.69 ms`
  (trainer-reported `2627.35 ms` total average), slower than the promoted
  source default and CUDA fallback diagnostics, so the temporary hook was
  removed.
- Rejected a temporary fused-forward-only swizzle hook with
  `LLMK_SM120_FORWARD_GELU_SUPER_M=5`, scoped to the `*_nt_bias_gelu` aliases
  while leaving non-GeLU forward, dInput, and dWeight aliases on their source
  defaults. The candidate passed `test_matmul` (`10/10`) and `test_attention`
  (all three smoke shapes), but the focused benchmark still left FC fused
  forward behind cuBLASLt (`1526.83 us` TK versus `1510.20 us`) and left qkv,
  attention-projection dWeight, FC-projection, and LM-head rows behind.
  TinyStories 3-step validation averaged `2696.77 ms` with steps `2734.11`,
  `2682.61`, and `2673.60 ms` (trainer-reported `2678.11 ms` total average),
  slower than the promoted source default and CUDA fallback diagnostics, so the
  temporary hook was removed.
...
- Rejected a temporary FC-projection-forward-only swizzle hook with
  ...
  FC-projection forward (`N == 768`, `K == 3072`) through a separate N96
  forward+bias alias while leaving qkv, fused FC, attention-projection,
  LM-head, dInput, and dWeight dispatch unchanged. The candidate passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark still left FC-projection forward behind cuBLASLt
  (`1412.91 us` TK versus `1386.87 us`) and left qkv, attention-projection
  dWeight, FC-projection dInput, and LM-head rows behind. TinyStories 3-step
  validation averaged `2624.16 ms` with steps `2622.82`, `2623.52`, and
  `2626.14 ms` (trainer-reported `2624.83 ms` total average), slower than the
  promoted source default and CUDA fallback diagnostics, so the temporary hook
  was removed.
- Rejected a temporary FC-projection-forward-only swizzle hook with
  `LLMK_SM120_FCPROJ_FORWARD_SUPER_M=5`. The source hook routed only GPT-2
  FC-projection forward (`N == 768`, `K == 3072`) through a separate N96
  forward+bias alias while leaving qkv, fused FC, attention-projection,
  LM-head, dInput, and dWeight dispatch unchanged. The candidate passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark still left FC-projection forward behind cuBLASLt
  (`1412.71 us` TK versus `1387.99 us`) and left qkv, attention-projection
  dWeight, FC-projection dInput, and LM-head rows behind. TinyStories 3-step
  validation averaged `2631.72 ms` with steps `2624.98`, `2625.24`, and
  `2644.94 ms` (trainer-reported `2635.09 ms` total average), slower than the
  promoted source default and CUDA fallback diagnostics, so the temporary hook
  was removed.
- Rejected a temporary FC-projection-forward-only swizzle hook with
  `LLMK_SM120_FCPROJ_FORWARD_SUPER_M=6`. The source hook routed only GPT-2
  FC-projection forward (`N == 768`, `K == 3072`) through a separate N96
  forward+bias alias while leaving qkv, fused FC, attention-projection,
  LM-head, dInput, and dWeight dispatch unchanged. The candidate passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark still left FC-projection forward behind cuBLASLt
  (`1412.68 us` TK versus `1382.64 us`) and left qkv, attention-projection
  dWeight, FC-projection dInput, and LM-head rows behind. TinyStories 3-step
  validation averaged `2622.73 ms` with steps `2620.59`, `2624.78`, and
  `2622.82 ms` (trainer-reported `2623.80 ms` total average), slower than the
  promoted source default and CUDA fallback diagnostics, so the temporary hook
  was removed.
- Rejected a temporary attention-projection-forward-only swizzle hook with
  `LLMK_SM120_ATTPROJ_FORWARD_SUPER_M=6`. The source hook routed only GPT-2
  attention-projection forward (`N == 768`, `K == 768`) through a separate
  N96 forward+bias alias while leaving qkv, fused FC, FC-projection, LM-head,
  dInput, and dWeight dispatch unchanged. The candidate passed `test_matmul`
  (`10/10`) and `test_attention` (all three smoke shapes), but the focused
  benchmark only tied the target row (`395.72 us` TK versus `392.63 us`
  cuBLASLt) and still left qkv dWeight, attention-projection dWeight,
  FC-projection dInput, and LM-head rows behind cuBLASLt. TinyStories 3-step
  validation averaged `2723.31 ms` over printed steps `2847.42`, `2681.79`,
  and `2640.71 ms` (trainer-reported `2661.25 ms` total average), slower than
  the promoted source default and CUDA fallback diagnostics, so the temporary
  hook was removed.
- Rejected a temporary attention-projection-forward-only swizzle hook with
  `LLMK_SM120_ATTPROJ_FORWARD_SUPER_M=5`. The source hook routed only GPT-2
  attention-projection forward (`N == 768`, `K == 768`) through a separate
  N96 forward+bias alias while leaving qkv, fused FC, FC-projection, LM-head,
  dInput, and dWeight dispatch unchanged. The candidate passed
  `test_attention` (all three smoke shapes) and `test_matmul` on rerun after
  the known intermittent MLP-up row. The focused benchmark rejected the target
  row (`422.71 us` TK versus `367.42 us` cuBLASLt) and still left qkv,
  attention-projection dWeight, FC-projection dInput, and LM-head rows behind
  cuBLASLt. TinyStories 3-step validation averaged `2624.68 ms` with steps
  `2620.58`, `2626.41`, and `2627.04 ms` (trainer-reported `2626.72 ms`
  total average), slower than the promoted source default and CUDA fallback
  diagnostics, so the temporary hook was removed.
- Rejected a temporary attention-projection-forward-only swizzle hook with
...
- Rejected a temporary FC-projection-forward-only swizzle hook with
  ...
  FC-projection forward (`N == 768`, `K == 3072`) through a separate N96
  forward+bias alias while leaving qkv, fused FC, LM-head, dInput, and dWeight
  dispatch unchanged. The candidate passed `test_attention` (all three smoke
  shapes) and `test_matmul` on rerun after the known intermittent MLP-up row,
  but the focused benchmark worsened the target row to `1410.76 us` TK versus
  `1355.83 us` cuBLASLt and regressed several adjacent rows. TinyStories
  3-step validation averaged `2618.91 ms` with steps `2616.57`, `2618.05`,
  and `2619.76 ms`, slower than the promoted source default and CUDA fallback
  diagnostics, so the temporary hook was removed.
- Rejected a temporary FC-projection-forward-only swizzle hook with
  `LLMK_SM120_FCPROJ_FORWARD_SUPER_M=2`. The source hook routed only GPT-2
  FC-projection forward (`N == 768`, `K == 3072`) through a separate N96
  forward+bias alias while leaving qkv, fused FC, LM-head, dInput, and dWeight
  dispatch unchanged. The candidate passed `test_matmul` (`10/10`) and
  `test_attention` (all three smoke shapes), but the focused benchmark still
  left the target row behind cuBLASLt (`1411.35 us` TK versus `1381.99 us`)
  and left qkv, attention-projection dWeight, and LM-head rows behind.
  TinyStories 3-step validation regressed to `2627.93 ms` average with steps
  `2613.98`, `2617.95`, and `2637.91 ms`, so the temporary hook was removed.
- Rejected a temporary qkv-forward-only swizzle route with
...
- Rejected a temporary fused-forward-only swizzle hook with
  ...
  while leaving plain forward, dInput, and dWeight aliases on their accepted
  defaults. The candidate passed `test_matmul` (`10/10`) and `test_attention`
  (all three smoke shapes), and the focused benchmark narrowed but did not
  close the fused FC forward gap (`1528.36 us` TK versus `1515.80 us`
  cuBLASLt). It still left qkv, attention-projection, FC-projection, and
  LM-head rows behind the CUDA variants. TinyStories 3-step validation
  regressed to `2649.37 ms` average with steps `2616.85`, `2666.39`, and
  `2632.34 ms`, so the temporary hook was removed.
- Rejected a temporary FC-projection-forward-only swizzle hook with
  `LLMK_SM120_FCPROJ_FORWARD_SUPER_M=8`. The source hook routed only GPT-2
  FC-projection forward (`N == 768`, `K == 3072`) through a separate N96
  forward+bias alias while leaving qkv, fused FC, LM-head, dInput, and dWeight
  dispatch unchanged. The candidate passed `test_matmul` (`10/10`) and
  `test_attention` (all three smoke shapes), but the focused benchmark
  regressed the target row to `1551.72 us` TK versus `1439.70 us` cuBLASLt
  and also left qkv/attention-projection/LM-head rows behind the CUDA
  variants. TinyStories 3-step validation regressed to `2692.81 ms` average
  with steps `2656.21`, `2704.26`, and `2681.35 ms`, so the temporary hook
  was removed.
...
- Rejected huge-N forward swizzle override
  ...
  (`24826.66 us` TK versus `22453.75 us`) and regressed attention-projection
  forward plus the fused dGELU row. TinyStories 3-step validation averaged
  `2636.77 ms` with steps `2637.15`, `2641.29`, and `2632.26 ms`, so huge-N
  forward stays on `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=15`. The macro build passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark still left LM-head forward behind cuBLASLt
  (`25155.76 us` TK versus `22776.83 us`) and worsened the fused FC path
  versus the accepted default. TinyStories 3-step validation regressed to
  `2696.17 ms` average with steps `2717.76`, `2683.01`, and `2709.32 ms`, so
  huge-N forward stays on `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
...
- Rejected huge-N forward swizzle override
  ...
  `25288.85 us` TK versus `22580.58 us` cuBLASLt. The run also left qkv
  forward, fcproj dInput, attproj dWeight, and LM-head rows behind the CUDA
  variants despite noisy smaller-row wins. TinyStories 3-step validation
  averaged `2636.12 ms` with steps `2630.85`, `2640.12`, and `2632.12 ms`,
  so huge-N forward stays on `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=12`. The macro build passed
  `test_attention`; the first `test_matmul` pass hit the known intermittent
  MLP-up forward failure (`5.7500` max diff versus `0.50` tolerance), and the
  immediate rerun passed `10/10`. The focused benchmark regressed the targeted
  LM-head forward row to `24950.23 us` TK versus `22066.34 us` cuBLASLt and
  also worsened fcproj forward. TinyStories 3-step validation averaged
  `2630.67 ms` with steps `2605.05`, `2622.54`, and `2638.81 ms`, so
  huge-N forward stays on `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=11`. The macro build passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark only nudged LM-head forward to `24439.88 us` TK versus
  `21973.80 us` cuBLASLt, still `1.11x` slower, while qkv/fcproj forward and
  dWeight-heavy rows remained behind the CUDA variants. TinyStories 3-step
  validation averaged `2621.49 ms` with steps `2621.11`, `2621.68`, and
  `2621.31 ms`, slower than the promoted source default, so huge-N forward
  stays on `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=2`. The macro build passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark worsened the targeted LM-head forward row
  (`24928.03 us` TK versus `21773.46 us` cuBLASLt) and left the same qkv,
  fcproj, dWeight, and LM-head gaps open. TinyStories 3-step validation
  averaged `2638.24 ms` with steps `2627.15`, `2632.98`, and `2643.49 ms`,
  so huge-N forward stays on `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
...
- Rejected scoped fused dInput+dGELU swizzle
  ...
  (`10/10`) and `test_attention` (all three smoke shapes), and the focused
  benchmark showed FC-projection dInput+dGELU slightly ahead of cuBLASLt in
  that run (`1884.96 us` versus `1908.29 us`), but the row was not better
  than the previously recorded no-override diagnostic timing and the broader
  GEMM gaps remained. TinyStories 3-step validation regressed to `2750.04 ms`
  average with steps `2745.59`, `2748.20`, and `2751.88 ms`, so fused dGELU
  aliases keep the inherited `LLMK_SM120_DINP_SUPER_M=8` default.
- Rejected huge-N forward swizzle override
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=8`. The macro build passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark worsened LM-head forward relative to cuBLASLt
  (`26648.88 us` TK versus `23094.77 us`) and still trailed on LM-head
  dInput/dWeight. TinyStories 3-step validation averaged `2728.49 ms` with
  steps `2724.66`, `2725.52`, and `2731.46 ms`; this beats the original
  llm.c baseline but remains slower than the current pure-TK source default
  and the CUDA/cuBLASLt fallback measurements. The source default remains
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=5`. The macro build passed
  `test_matmul` (`10/10`) and `test_attention` (all three smoke shapes), but
  the focused benchmark left LM-head forward behind cuBLASLt (`26394.98 us`
  TK versus `23228.09 us`) and still trailed on LM-head dInput/dWeight.
  TinyStories 3-step validation averaged `2741.33 ms` with steps `2728.05`,
  `2750.29`, and `2732.38 ms`, slower than the current pure-TK source default
  and the CUDA/cuBLASLt fallback measurements. The source default remains
  `LLMK_SM120_HUGE_N_FORWARD_SUPER_M=7`.
- Rejected huge-N forward swizzle override
...
- Added `LLMK_SM120_HUGE_N_FORWARD_WARPS4` as a default-off hook that changes
  ...
  `test_attention`, but `test_matmul` failed the LM-head forward row with max
  diff `19.9062` versus tolerance `0.50`, so no training run was attempted for
  the numerically invalid kernel. The no-override source build passed
  `test_attention` and `test_matmul` (`10/10`), leaving the default huge-N
  forward tile on 8 warps.
- Rejected huge-N forward swizzle override
