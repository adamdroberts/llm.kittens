...
parser = argparse.ArgumentParser(
    description="Run Nsight Compute on profile_gpt2cu and summarize goal.md profiling metrics."
)
parser.add_argument("--binary", default="./profile_gpt2cu", help="Profiling binary to run under ncu")
parser.add_argument("--output", default="profile", help="ncu output basename passed to -o")
parser.add_argument("--report", default=None, help="Existing or generated .ncu-rep path")
parser.add_argument("--csv-input", default=None, help="Existing Nsight Compute raw CSV export; skips build/run/ncu export")
parser.add_argument("--skip-build", action="store_true", help="Do not rebuild profile_gpt2cu before profiling")
parser.add_argument("--skip-run", action="store_true", help="Do not run ncu; only summarize an existing report")
parser.add_argument("--min-tensor-util", type=float, default=70.0,
                    help="Minimum average tensor-core utilization percentage; set <=0 to disable")
parser.add_argument("--gelu-fusion", type=int, choices=(0, 1), default=0,
                    help="Pass --gelu-fusion to profile_gpt2cu; 1 profiles the opt-in bias+GELU epilogue")
args = parser.parse_args()

report_path = args.report if args.report is not None else f"{args.output}.ncu-rep"
...
# build the executable. llm.kittens removes cuDNN/cuBLAS entirely, so keep the
# profiling build on the same TK-only single-process path as the docs.
if not args.skip_build and args.csv_input is None:
...
if args.csv_input is not None:
    if not os.path.exists(args.csv_input):
        raise FileNotFoundError(f"--csv-input does not exist: {args.csv_input}")
elif not args.skip_run:
    # try to see if profiling is allowed for non-root:
    try:
        options = subprocess.check_output(["modprobe", "-c", "nvidia"], text=True, stderr=subprocess.DEVNULL)
        can_profile = any("NVreg_RestrictProfilingToAdminUsers=0" in l for l in options.splitlines())
    except (FileNotFoundError, subprocess.CalledProcessError):
        print("Could not inspect NVreg_RestrictProfilingToAdminUsers; trying ncu without sudo.", file=sys.stderr)
        can_profile = True

    # record metrics
    # --full and --import-source are entirely superfluous for this script, but you might want to
    # manually inspect the generated .ncu-rep, so we keep it here.
    cmd = [
        NCU, "--set", "full", "--import-source", "yes", "-o", args.output,
    ...
    raise FileNotFoundError(f"--skip-run requested, but report does not exist: {report_path}")
...
min_tensor_util = args.min_tensor_util
print("." * len(units))
...
if min_tensor_util > 0.0:
    print(f"Tensor-core utilization gate: {avg_tensor_util:.1f}% >= {min_tensor_util:.1f}%")
    if avg_tensor_util < min_tensor_util:
        raise SystemExit(
            f"average tensor-core utilization {avg_tensor_util:.1f}% is below "
            f"the required {min_tensor_util:.1f}%"
        )
